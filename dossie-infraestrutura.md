...
- **Mensagem nova reabre chamado finalizado** (2026-09-24): `RESOLVIDO` → `ABERTO` automaticamente, na mesma transação do envio, venha a mensagem do lojista ou do suporte. Se um chamado "se reabrir sozinho", é isso (conferir a última mensagem dele) — inclusive quando o próprio admin manda mensagem depois de marcar Resolvido.
...
- **"ID da Loja" vs. código pessoal** (2026-09-24): para identificar o cliente no suporte, use o **ID da Loja** (`empresas.codigo_loja`, 5 dígitos) — é o que **todos** os usuários da empresa veem na tela de Suporte e o que aparece no Painel Master (aba Empresas, chamados e chat). Cada usuário também tem um **código pessoal** (`usuarios.codigo_usuario`), que não é mais exibido no Suporte. O ID da Loja de uma empresa antiga é o código pessoal do admin fundador (por isso o dono vê o mesmo número nos dois). Para achar a loja pelo número: `SELECT id, razao_social FROM empresas WHERE codigo_loja = '12176';`.
...
- **Backup do banco — comando validado (2026-09-24)**, usa a senha que já está no ambiente do container (não digita, não fica no histórico do shell):
  ```bash
  ...
  tail -n 1 backup_*.sql   # tem que terminar em "-- Dump completed on ..."
  ```
  As aspas **simples** são essenciais (a variável é expandida dentro do container, não no host). **Não use** `docker exec sae_mysql mysqldump -uroot -p ... > arquivo`: sem `-i` o prompt de senha lê vazio → `ERROR 1045 ... (using password: NO)`; e com `-t` o prompt/caracteres de controle vão parar dentro do arquivo de backup, corrompendo-o. Se o root tiver sido trocado depois da criação do container (a variável fica desatualizada → 1045), gere o dump dentro do container e copie: `docker exec -it sae_mysql sh -c 'mysqldump -uroot -p --single-transaction --routines --triggers sae > /tmp/bkp.sql'` → `docker cp sae_mysql:/tmp/bkp.sql ./backup.sql` → `docker exec sae_mysql rm /tmp/bkp.sql`.
- **Todo redeploy com migration nova: aplicar `migrate deploy` LOGO EM SEGUIDA** — com `pull_policy: build` o código novo da API sobe na hora e, se ele ler colunas que ainda não existem, as rotas respondem 500 até a migration rodar (aconteceu em 2026-09-24: o hook de acesso lê `usuarios.ativo` em toda requisição). Ordem: backup → redeploy → `docker exec -it sae_api npx prisma migrate deploy` → `docker restart sae_api`.
- **`migrate deploy` com erro P3009 ("found failed migrations")** — uma tentativa anterior quebrou no meio e deixou a linha em `_prisma_migrations` com `finished_at` nulo; o Prisma bloqueia TODAS as migrations seguintes até isso ser resolvido. Como o MySQL não desfaz DDL, parte da migration pode ter ficado aplicada. Ordem: ver o erro real (`SELECT migration_name, logs FROM _prisma_migrations WHERE finished_at IS NULL\G`) → conferir quais objetos da migration existem → corrigir/completar ou desfazer à mão → `docker exec -it sae_api npx prisma migrate resolve --applied <nome>` (banco = fim da migration) **ou** `--rolled-back <nome>` (banco = antes dela; o deploy a roda de novo) → `migrate deploy`. Nunca marcar `--applied` sem conferir. Caso real: 2026-09-24, migration do chat de suporte.
0. **Comandos do Prisma no servidor rodam DENTRO do container**: o host de produção não tem Node.js (`npx: command not found`). Sempre `docker exec -it sae_api npx prisma migrate status` / `... migrate deploy` — e **nunca `migrate dev`** em produção (ver 4.3).
...
- **Prisma exige banco "shadow" para `migrate dev`** (não para `db push` nem `migrate deploy`) — o usuário de aplicação (`sae_app`) só tem grants no banco `sae`, sem `CREATE DATABASE`. Sem `SHADOW_DATABASE_URL` definida, `migrate dev` falha com **P3014 + P1010**. Soluções (validadas em 2026-09-24, ver `NOTAS_IMPORTANTES.md`): **(a, recomendada)** criar a base sombra com o root e dar ao `sae_app` direitos só nela — `CREATE DATABASE sae_shadow CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci; GRANT ALL PRIVILEGES ON sae_shadow.* TO 'sae_app'@'%';` + `SHADOW_DATABASE_URL="mysql://sae_app:<senha>@localhost:3306/sae_shadow"`; **(b)** grant por padrão de nome ``GRANT ALL PRIVILEGES ON `prisma\_migrate\_shadow\_db\_%`.* TO 'sae_app'@'%';`` (enviar por stdin — `mysql -e` quebra no `\_`). **Nunca** `GRANT CREATE ON *.*` global para o usuário da app. E **nunca `migrate dev` em produção** — lá é `migrate deploy`, que não usa sombra.
...
- **Deadlock InnoDB em "INSERT no filho + UPDATE no pai" dentro da mesma transação** — o INSERT numa tabela com FK pega lock compartilhado (S) na linha do pai; um UPDATE nessa mesma linha logo depois precisa do lock exclusivo (X). Com duas transações concorrentes no mesmo pai, cada uma segura S e espera o X da outra → o MySQL aborta uma (`P2034` no Prisma, vira 500). Já aconteceu de verdade no envio de mensagens do chat de suporte (2026-09-24, `chatChamado.service.js#enviarMensagem`). Regra: **atualize o pai ANTES de inserir o filho** (a transação pega o X logo de início e as outras esperam na fila). Se um `P2034` aparecer no log do `api`, procure esse padrão primeiro.
