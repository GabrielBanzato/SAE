 ...
 * (direita). O carrinho e so estado local do React - o backend nao tem
 * (nem foi pedido) um endpoint de "venda com multiplos itens", entao
 * "Finalizar Venda" dispara uma chamada `POST /vendas` por item do
 * carrinho, em sequencia (ver `finalizarVenda` abaixo). Isso reaproveita
 * a rota que ja existe (1 produto por venda, decisao de modelagem da
 * Parte 1) em vez de inventar uma rota nova nao pedida nesta tarefa.
 */
export default function Vendas() {
  ...
  useEffect(() => {
    ...
    // Erro ao carregar clientes nao deve travar o PDV - vender sem
    // cliente atrelado continua funcionando normalmente.
    apiFetch('/clientes')
      .then((dados) => {
  ...
  async function finalizarVenda() {
    ...
    // Processa 1 item por vez (a API so aceita 1 produto por venda). Para
    // no primeiro erro, em vez de continuar tentando os proximos - assim
    // fica claro exatamente quais itens ja foram vendidos de verdade e
    // quais ainda precisam ser reenviados (o carrinho mantem so os que
    // faltam, o usuario pode ajustar e clicar em "Finalizar" de novo).
    const restantes = [...carrinho];
    let vendidos = 0;
    ...
    while (restantes.length > 0) {
      ...
      try {
        ...
        // Atualiza o estoque exibido no catalogo sem precisar recarregar
        // a lista inteira (produtos sob demanda nunca debitam estoque no
        // backend, entao nao mexe neles aqui tambem).
        setProdutos((atual) =>
          atual.map((produto) =>
            produto.id === item.produtoId && !produto.sobDemanda
              ? { ...produto, estoqueAtual: produto.estoqueAtual - item.quantidade }
              : produto
          )
        );
      } catch (err) {
        setErroVenda(
          `${vendidos > 0 ? `${vendidos} item(ns) já registrado(s) com sucesso. ` : ''}Falha ao vender "${item.nome}": ${err.message || 'Erro desconhecido.'}`
        );
        break;
      }
    }
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_400px]">
        ...
        <div className="flex flex-col rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 dark:bg-slate-800 dark:ring-slate-700">
          ...
          <div className="mt-4 shrink-0 border-t border-slate-200 pt-4 dark:border-slate-700">
            ...
            {sucessoVenda && (
              <p className="mt-3 rounded-2xl bg-emerald-50 p-3 text-sm font-medium text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">
                {sucessoVenda}
              </p>
            )}
            ...
            <button
              ...
              className="mt-4 flex w-full items-center justify-center gap-3 rounded-2xl bg-emerald-600 px-6 py-4 text-xl font-bold text-white shadow-lg shadow-emerald-600/20 transition-colors hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
            >
